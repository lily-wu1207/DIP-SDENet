...
def get_args_parser():
    ...
    parser.add_argument('--data_path', default='/content/drive/MyDrive/test/FSC/datasets/FSC', type=str,
                        help='dataset path')
    parser.add_argument('--output_dir', default='./data/out/fim_dir',
    ...
    # Logging parameters
    parser.add_argument('--log_dir', default='./logs/fim_dir',
...
def main(args):
    ...
    dataset_train = FSC147(args.data_path, t="train")
    print(dataset_train)
    ...
    # define the model
    model = SDCAT.__dict__[args.model]()
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, gt_density, boxes, m_flag, scale_embed) in enumerate(
            ...
            scale_embed = torch.LongTensor(scale_embed).to(device, non_blocking=True)
            shot_num = 3
            with torch.cuda.amp.autocast():
                output = model(samples, boxes, shot_num,scale_embed)  # 训练
