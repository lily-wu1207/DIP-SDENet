...
class ResizeSomeImage(object):
    def __init__(self, data_path='./data/FSC147'):
...
class ResizeTrainImage(ResizeSomeImage):
    ...
    def __call__(self, sample):
        image, lines_boxes, density, dots, im_id, m_flag, scale_embed = sample['image'], sample['lines_boxes'], sample['gt_density'], \
            sample['dots'], sample['id'], sample['m_flag'], sample['scale_embed']
        ...
        for box in lines_boxes:
            ...
            # print("box")
            # plt.imshow(bbox[0].numpy())
            # plt.show()
        boxes = np.array(boxes)
        boxes = torch.Tensor(boxes)

        # boxes shape [3,3,64,64], image shape [3,384,384], density shape[384,384]
        sample = {'image': reresized_image, 'boxes': boxes, 'gt_density': reresized_density, 'm_flag': m_flag, 'scale_embed': scale_embed}
